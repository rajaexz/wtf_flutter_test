...
class CallNotifier extends AsyncNotifier<CallStateData> implements HMSUpdateListener {
  ...
  Future<void> joinRoom(String roomId, String callRequestId) async {
    ...
    logger.rtc('[CALL] trainer joining room $roomId');

    try {
      final token = await _fetchToken(user.id, 'trainer');
  ...
  Future<void> switchCamera() async => _sdk?.switchCamera();
  ...
  Future<void> endCall(String callRequestId) async {
    ...
    if (user != null) {
      ...
      logger.rtc('[CALL] trainer session log saved');
    }
  ...
  @override
  void onJoin({required HMSRoom room}) => logger.rtc('[CALL] trainer joined ${room.id}');
  ...
  @override
  void onReconnected() => logger.rtc('[CALL] trainer reconnected');

  @override
  void onReconnecting() => logger.rtc('[CALL] trainer reconnecting...');
  ...
  void onHMSError({required HMSException error}) {
    logger.rtc('[CALL] trainer hms error: ${error.message}');
    state = AsyncData(CallStateData(state: CallState.error, errorMessage: error.message));
  }
}
